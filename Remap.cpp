		...
		for (int thisX = 0; thisX < (Width); thisX++) {// from here if something is enabled then draw it 
				...
				| flip == 0xc)
			{
				...
				case 0x4:
					...
					if (mappingId == 0xbaad)
					{
						mappingId = newMapData.size() - 1;
						newMapData.push_back(tmpMappings);
						newTileTableMappingIndexes[mappingId] = TILE;
					}
					break;
				case 0x8:
					for (int tileCounter = 0; tileCounter < 4; tileCounter++)
